...
export class CnyAlipayStrategy extends CnyBaseStrategy {
  ...
  private extractPhotoOrThrow(ctx: CustomSceneContext): PhotoSize {
    ...
    throw new MissingAttachmentError(
      'Пожалуйста, прикрепите фотографию QR-кода Alipay получателя.',
    );
  }
  ...
  private async downloadPhotoBuffer(
    ...
    try {
      ...
      console.error('[CnyAlipayStrategy] Unable to download photo from message', error);
      throw new MissingAttachmentError(
        'Не удалось загрузить фотографию QR-кода Alipay. Попробуйте отправить заявку снова.',
      );
    }
  }
  ...
  private async savePhotoToDatabase(
    ...
    try {
      ...
      console.log(`[CnyAlipayStrategy] Photo path stored in scene state: ${photoPath}`);
    } catch (error) {
      console.error('[CnyAlipayStrategy] Failed to save photo path to scene state', error);
