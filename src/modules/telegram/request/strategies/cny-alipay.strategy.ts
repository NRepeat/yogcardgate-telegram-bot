...
interface CnyAlipayParsedInput extends ParsedStrategyInput {
  identifier?: string;
  recipient?: string;
}

export class CnyAlipayStrategy extends CnyBaseStrategy {
  constructor(deps: CnyStrategyDependencies) {
    super(deps);
  }

  protected supportsMethod(method: PaymentMethodEnum): boolean {
    return method === PaymentMethodEnum.CNY_ALIPAY;
  }
  ...
  protected async createRequest({
    ...
    rate,
    parsed,
  }: CreateRequestParams & { parsed: CnyAlipayParsedInput }): Promise<FullRequestType> {
    ...
    const request = await this.deps.requestService.createGeneralRequest({
      ...
      method: {
        ...
        qr: {
          identifier: parsed.identifier || 'Alipay',
  ...
  protected buildDetails(data: CnyAlipayParsedInput): string {
    const lines = [
      'Тип: CNY Alipay',
  ...
  private extractPhotoOrThrow(ctx: CustomSceneContext): PhotoSize {
    ...
    throw new MissingAttachmentError(
      'Пожалуйста, отправьте фотографию Alipay.',
    );
  }
  ...
  private async downloadPhotoBuffer(
    ...
    try {
      ...
      console.error('[CnyAlipayStrategy] Unable to download photo', error);
      throw new MissingAttachmentError(
  ...
  private async savePhotoToSceneState(
    ...
    try {
      ...
      console.error('[CnyAlipayStrategy] Failed to save photo path', error);
    }
  }
}
