...
export interface TryStrategyDependencies {
  ratesService: RatesService;
  requestService: RequestService;
  vendorService: VendorService;
}

export abstract class TryBaseStrategy implements PaymentRequestStrategy {
  protected readonly targetCurrency = CurrencyEnum.TRY;

  constructor(protected readonly deps: TryStrategyDependencies) {}
  ...
  async execute(
    ...
    try {
      ...
      for (const item of parsed.data) {
        const rate = this.findRate(rates, item.amount, method);
        if (!rate) {
          return {
            status: 'error',
      ...
      console.error('[TRY Strategy] Unexpected error:', error);
      return {
        status: 'error',
